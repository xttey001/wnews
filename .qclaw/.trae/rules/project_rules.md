...
> ⚠️ **红线警告：所有分析必须基于真实搜索数据，禁止幻想、编造或推测**

#### 必须搜索的真实数据类型

| 数据类型 | 搜索工具 | 使用场景 |
|----------|----------|----------|
| **股市行情数据** | WebSearch | 指数点位、涨跌幅、成交额 |
| **个股数据** | WebSearch | 股价、涨停、成交额、龙虎榜 |
| **宏观指标** | WebSearch | 美元指数、美债利率、VIX等 |
| **财经新闻** | WebSearch | 政策发布、公司公告、行业动态 |
| **板块资金流向** | WebSearch | 北向资金、主力净流入、ETF申赎 |
